...
public final class KeystrokesRenderer {
    ...
    public static void render(GuiGraphicsExtractor ctx, float x, float y, float scale, KeyStates k) {
        HudConfig c = HudConfig.get();
        int w = width();
    ...
    private static void drawMovement(GuiGraphicsExtractor ctx, HudConfig c, int y, KeyStates k) {
        int b = c.boxSize;
    ...
    private static void drawClicks(GuiGraphicsExtractor ctx, HudConfig c, int w, int y, KeyStates k) {
        int mouseW = (w - GAP) / 2;
    ...
    private static void drawSpace(GuiGraphicsExtractor ctx, HudConfig c, int w, int y, KeyStates k) {
        int sh = spaceHeight();
    ...
    private static void drawKey(GuiGraphicsExtractor ctx, String label, String subLabel, int x, int y, int w, int h, float fade) {
        HudConfig c = HudConfig.get();
        ...
        if (subLabel == null) {
            int lx = Math.round(x + (w - tr.width(label)) / 2f);
            ctx.text(tr, label, lx, y + (h - 9) / 2 + 1, txt, c.textShadow);
        } else {
            float s2 = 0.7f;
            float gap = 1f;
            ...
            int lx = Math.round(x + (w - tr.width(label)) / 2f);
            ctx.text(tr, label, lx, Math.round(top), txt, c.textShadow);
            ctx.pose().pushMatrix();
            ctx.pose().translate(x + (w - tr.width(subLabel) * s2) / 2f, top + 9 + gap);
            ctx.pose().scale(s2, s2);
            ctx.text(tr, subLabel, 0, 0, txt, c.textShadow);
            ctx.pose().popMatrix();
        }
    }
