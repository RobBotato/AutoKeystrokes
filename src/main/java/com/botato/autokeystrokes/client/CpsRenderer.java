...
public final class CpsRenderer {
    ...
    private record Seg(String text, int color, int padLeft, int padRight) {
        ...
        int totalWidth(Font tr) { return padLeft + tr.width(text) + padRight; }
    }

    private static Font tr() {
    ...
    public static void render(GuiGraphicsExtractor ctx, float x, float y, float scale) {
        HudConfig c = HudConfig.get();
        Font tr = tr();
        int w = width();
        int h = height();
        ...
        for (Seg s : segs) {
            sx += s.padLeft();
            ctx.text(tr, s.text(), sx, ty, s.color(), c.cpsTextShadow);
            sx += tr.width(s.text()) + s.padRight();
        }

        ctx.pose().popMatrix();
    }

    private static void drawBorder(GuiGraphicsExtractor ctx, int x, int y, int w, int h, int t, int col) {
