...
public class AutoKeystrokesClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        String category = "key.categories.autokeystrokes";
    ...
    public class CPSConfigScreen extends Screen {
        ...
        private void drawEditor(DrawContext ctx, HudModule mod, String name, int w, int h) {
            ...
            ctx.getMatrices().translate(mod.x + (w * mod.scale) + 2, mod.y, 0.0f);
            ctx.getMatrices().scale(mod.scale * (name.equals("Keys") ? 0.9f : 0.6f), mod.scale * (name.equals("Keys") ? 0.9f : 0.6f), 1.0f);
            ...
            if (isSelected) {
                ...
                ctx.getMatrices().translate(cX - ((textRenderer.getWidth(label) * lblSc)/2), mod.y + (h * mod.scale) + 2, 0.0f);
                ctx.getMatrices().scale(lblSc, lblSc, 1.0f);
