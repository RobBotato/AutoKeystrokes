...
public class AutoKeystrokesClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        String category = "category.autokeystrokes";
    ...
    public class CPSConfigScreen extends Screen {
        ...
        private void drawEditor(DrawContext ctx, HudModule mod, String name, int w, int h) {
            ...
            ctx.getMatrices().translate(mod.x + (w * mod.scale) + 2, mod.y, 0);
            ctx.getMatrices().scale(mod.scale * (name.equals("Keys") ? 0.9f : 0.6f), mod.scale * (name.equals("Keys") ? 0.9f : 0.6f), 1f);
            ...
            if (isSelected) {
                ...
                ctx.getMatrices().translate(cX - ((textRenderer.getWidth(label) * lblSc)/2), mod.y + (h * mod.scale) + 2, 0);
                ctx.getMatrices().scale(lblSc, lblSc, 1f);
        ...
        public boolean mouseClicked(double mx, double my, int btn) {
            ...
            if (panel.open && resetButton.mouseClicked(mx, my, btn)) return true;
        ...
        @Override public boolean mouseScrolled(double mx, double my, double va) {
            HudConfig cfg = HudConfig.get();
        ...
        @Override public boolean mouseReleased(double mx, double my, int btn) {
            panel.mouseReleased();
            if (btn == 0) { dragging = false; vGuide = null; hGuide = null; HudConfig.save(); }
        ...
        public boolean charTyped(char chr, int modifiers) {
            if (panel.open && panel.charTyped(chr)) return true;
            return super.charTyped(chr, modifiers);
        }

        @Override
        public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
            if (panel.open && panel.keyPressed(keyCode)) return true;
