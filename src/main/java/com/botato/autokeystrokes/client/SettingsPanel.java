...
public class SettingsPanel {
    ...
    public void render(GuiGraphicsExtractor ctx, int screenWidth, int screenHeight, int mouseX, int mouseY) {
        if (!open) return;
        ...
        if (tooltip != null) {
            int w = tr.width(tooltip) + 6;
            int tx = Math.min(mouseX + 8, screenW - w - 2);
            int ty = mouseY - 14;
            ctx.fill(tx - 1, ty - 1, tx + w + 1, ty + 12, 0xFF2A2A2A);
            ctx.fill(tx, ty, tx + w, ty + 11, 0xF0100010);
            ctx.text(tr, tooltip, tx + 3, ty + 2, 0xFFE0E0E0, false);
        }
    }

    private void renderRow(GuiGraphicsExtractor ctx, Font tr, Entry e, int y, int mouseX, int mouseY) {
        int labelX = x0 + PAD;
        ...
        switch (e.kind()) {
            ...
            case SLIDER -> {
                ...
                ctx.text(tr, val, x1 - PAD - tr.width(val), y + 3, 0xFFAAAAAA, false);
            ...
            case COLOR -> {
                ...
                int swX = x1 - PAD - 10;
                ctx.text(tr, hex, swX - 4 - tr.width(hex), y + 3, 0xFF888888, false);
    ...
    private void renderPicker(GuiGraphicsExtractor ctx, Font tr, int top) {
        ...
        ctx.text(tr, hex, pkHexX, pkHexY, hexCol, false);
    ...
    private void drawRing(GuiGraphicsExtractor ctx, int cx, int cy) {
